# This is the third version of the requested random test data creator for 2024s
...
#
# This is an updated version of RandomData2024B.py which no longer generates
# the now obsolete items of:
#
#    autoAmpMiss,
#    teleAmpMiss,
#    notNear,
...
AMP = 7                     # (Teleop) Amp score count
SPEAKER = 8                 # (Teleop) Speaker score count
SPEAKERMISS = 9             # (Teleop) Speaker miss count

# The actual team array
a_Teams = [
    # num  name                  ASCF    ARF     TSCF    TRF     MATCH...
    [60,   "Bulldogs",           -0.2,   0.1,    0.3,    0.4,    1, 0, 0, 0],
    [498,  "Cobra Commanders",   0.7,    0.5,    2.1,    1.5,    1, 0, 0, 0],
    [698,  "Hamilton Microbots", 2,      6,      -0.2,   0.3,    1, 0, 0, 0],
    [991,  "BroncoBots",         -1,     0,      -1,     3,      1, 0, 0, 0],
    [1165, "Team Paradise",      0.8,    2,      0.8,    1.1,    1, 0, 0, 0],
    [1212, "Sentinels",          -2,     1.5,    0.5,    1.6,    1, 0, 0, 0],
    [1726, "N.E.R.D.S.",         0.5,    2.1,    0.9,    4,      1, 0, 0, 0],
    [2262, "RoboKrew",           0.4,    0.5,    0.76,   0.8,    1, 0, 0, 0],
    [2403, "Plasma",             0.5,    0.3,    1.8,    1.1,    1, 0, 0, 0],
    [2478, "Westwood",           1.2,    1,      1.2,    0.8,    1, 0, 0, 0],
    [2486, "CocoNuts",           1,      0,      2,      1.3,    1, 0, 0, 0],
    [3944, "All Knights",        0,      1,      0,      1.9,    1, 0, 0, 0],
    [4146, "Sabercats",          0.1,    4,      0.1,    0.7,    1, 0, 0, 0],
    [4183, "Bit Buckets",        0.2,    3,      0.2,    1.1,    1, 0, 0, 0],
    [5539, "DVHS Cyborgs",       0.2,    0.8,    -0.2,   0.8,    1, 0, 0, 0],
    [6352, "LAUNCH TEAM",        0.5,    0.2,    0.5,    0.2,    1, 0, 0, 0],
    [6413, "Degrees of Freedom", 2,      1,      1.8,    2,      1, 0, 0, 0],
    [6479, "AZTECH",             0.5,    0.6,    1.5,    0.6,    1, 0, 0, 0],
    [6656, "Ryu Botics",         0.76,   1.2,    0.75,   1,      1, 0, 0, 0],
    [6833, "Phoenix",            -0.5,   0,      -0.5,   0.4,    1, 0, 0, 0],
    [8021, "Panther Robotics",   0.4,    2,      0.4,    0.8,    1, 0, 0, 0],
    [8087, "Cougar Pride",       0,      1,      0,      1,      1, 0, 0, 0],
    [8848, "Blu CREW",           0.8,    2,      0.8,    0.7,    1, 0, 0, 0],
    [9059, "COLTech",            0.6,    0.4,    0.7,    1,      1, 0, 0, 0]
    ]
...
for team in range(len(a_Teams)):
    ...
    a_Teams[team][SPEAKER] = randint(0, 10)
...
for matchNum in range(1, 41):
    ...
    for team in range(6):
        ...
        autoSpeaker         = calcNewScore(randint(0, 1), a_Teams[team][AUTOSCORECHANGEFACTOR], a_Teams[team][AUTORANDOMIZEFACTOR])
        ...
        a_Teams[team][SPEAKER]     = calcNewScore(a_Teams[team][SPEAKER], a_Teams[team][TELESCORECHANGEFACTOR], a_Teams[team][TELERANDOMIZEFACTOR])
        ...
        teleTrapMiss    = 0 if telePark else randint(0, 4 - teleTrap)
        teleMic         = randint(0, 3)
        teleMicMiss     = randint(0, 3 - teleMic)
        ...
        matchResults = (
            ...
            f'"a1":{autoSpeaker},"a2":{autoSpeakerMiss},"a3":{autoAmp},'
            f'"t1":{a_Teams[team][SPEAKER]},"t2":{a_Teams[team][SPEAKERMISS]},'
            f'"t3":{a_Teams[team][AMP]},"t4":{teleClimb},"t5":{teleTrap},"t6":{teleTrapMiss},'
            f'"m":{teleMic},"mm":{teleMicMiss},'
            f'"p":{telePark},"c":0,"ns":0,"d":0,"do":0,'
            f'"co":"{comments}"'
            )
